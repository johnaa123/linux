...
static int jz4740_wdt_ping(struct watchdog_device *wdt_dev)
{
	...
	regmap_write(drvdata->map, REG_WDT_TCNT, 0);
	return 0;
}
				    ...
				    unsigned int new_timeout)
{
	...
	regmap_read(drvdata->map, REG_WDT_TCER, &tcer);
	regmap_write(drvdata->map, REG_WDT_TCER, 0);
	...
	regmap_write(drvdata->map, REG_WDT_TDR, (u16)timeout_value);
	regmap_write(drvdata->map, REG_WDT_TCNT, 0);

	wdt_dev->timeout = new_timeout;

done:
	regmap_write(drvdata->map, REG_WDT_TCER, tcer & BIT(0));
	return ret;
}
...
static int jz4740_wdt_start(struct watchdog_device *wdt_dev)
{
	...
	if (ret)
		...
		regmap_write(drvdata->map, REG_WDT_TCER, 1);
