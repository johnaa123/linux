...
static int jz4740_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	/* Enable PWM output */
	regmap_update_bits(jz->map, REG_TCSRc(pwm->hwpwm),
				TCSR_PWM_EN, TCSR_PWM_EN);

	/* Start counter */
	regmap_write(jz->map, REG_TESR, BIT(pwm->hwpwm));
	return 0;
}
...
static void jz4740_pwm_disable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	 ...
	 */
	regmap_update_bits(jz->map, REG_TCSRc(pwm->hwpwm), TCSR_PWM_EN, 0);

	/* Stop counter */
	regmap_write(jz->map, REG_TECR, BIT(pwm->hwpwm));
}
			     ...
			     int duty_ns, int period_ns)
{
	...
	regmap_read(jz4740->map, REG_TER, &tcsr);
	is_enabled = tcsr & BIT(pwm->hwpwm);
	if (is_enabled)
		jz4740_pwm_disable(chip, pwm);

	/* Set abrupt shutdown */
	regmap_update_bits(jz4740->map, REG_TCSRc(pwm->hwpwm),
				TCSR_PWM_SD, TCSR_PWM_SD);

	/* Reset counter to 0 */
	regmap_write(jz4740->map, REG_TCNTc(pwm->hwpwm), 0);

	/* Set duty */
	regmap_write(jz4740->map, REG_TDHRc(pwm->hwpwm), duty);

	/* Set period */
	regmap_write(jz4740->map, REG_TDFRc(pwm->hwpwm), period);
		...
		struct pwm_device *pwm, enum pwm_polarity polarity)
{
	...
	case PWM_POLARITY_NORMAL:
		regmap_update_bits(jz->map, REG_TCSRc(pwm->hwpwm),
				TCSR_PWM_INITL_HIGH, 0);
		break;
	case PWM_POLARITY_INVERSED:
		regmap_update_bits(jz->map, REG_TCSRc(pwm->hwpwm),
				TCSR_PWM_INITL_HIGH, TCSR_PWM_INITL_HIGH);
		break;
	}
